...
const products = [
  ...
  { name: "Expensely", label: "EXPENSE TRACKING", icon: "◔", status: "LIVE", desc: "Track daily expenses, manage suppliers and products, and keep a clear picture of where your money goes.", features: ["Daily expense tracking", "Supplier & product management", "AI-powered insights"] },
  { name: "Mechanic AI", label: "SERVICE MANAGEMENT", icon: "⚙", status: "LIVE", desc: "Run a car mechanic shop with ease—manage jobs, customers and vehicles in one streamlined workspace.", features: ["Job & repair tracking", "Customer & vehicle records", "Streamlined workflow"] },
...
export const Products: React.FC = () => {
  const open = (name: string) => {
    if (name === "Invygo") return window.open(tokenService.getToken() ? "/dashboard" : "/login", "_blank", "noopener,noreferrer");
    if (name === "Expensely") return window.open("https://expensemanager-1-3sb6.onrender.com/", "_blank", "noopener,noreferrer");
    if (name === "Mechanic AI") return window.open("https://mechanic-ai-ui.onrender.com/", "_blank", "noopener,noreferrer");
    if (name === "ExchangeRate API") return window.open("https://exchangerate-3.onrender.com/docs", "_blank", "noopener,noreferrer");
  };
